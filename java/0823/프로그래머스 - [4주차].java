...
class Solution {
    public String solution(String[] table, String[] languages, int[] preference) {
        ...
        int ans;
       	...
       	String[][] datas = new String[table.length][];
        ...
        Arrays.sort(datas, Comparator.comparing(o1 -> o1[0])); 
        
        int point[] = new int[datas.length];
        
        for(int i=0; i<datas.length; i++){
            for(int j=1; j<datas[0].length; j++){
                for(int k=0; k<preference.length; k++){
                    if(datas[i][j].equals(languages[k])){
                        point[i] += ((datas[0].length-j)*preference[k]);
                    } 
                }
            }
        }
