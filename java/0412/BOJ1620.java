...
2 초	256 MB	21480	6748	4754	31.611%
문제
...
public class Main {
    ...
    // 보면 문제가 숫자인 경우, 문자인 경우 해당하는 value를 찾아내면 된다.
    // 따라서 그냥 순서를 바꿔서 숫자일 떄는 inthm, 문자일 때는 strhm으로 하면 된다.
    ...
    // 결과를 찾는다.
      ...
      // 집어넣은 값이 문자가 아닌경우(대문자만 있다고 했으니 A~Z외의 문자이면 다 숫자일 것이다.
			if(check.charAt(0)<'A' || check.charAt(0) > 'Z') {
        // 그 key를 사용하는데, 값이 0부터 들어갔기 때문에 1부터 세는 이 경우 -1를 해준다.
				System.out.println(inthm.get(Integer.parseInt(check)-1));
			}else {
        // 반대로 이 경우는 구한 값에서 1을 더하면 된다.
				System.out.println(strhm.get(check)+1);
			}
		}
